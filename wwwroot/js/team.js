...
function LoadGrids(rosterData) {
    let guildMembers = rosterData.members.filter(function(member) {
        return member.rank === Roles.GuildLeader || member.rank === Roles.Officer || member.rank === Roles.RaidLeader || member.rank === Roles.Banker || member.rank === Roles.Raider;
    });
    ...
    GetMembers(guildMembers, RETRIES);
    
}

function GetMembers(guildMembers, retries) {
    GetMembersData(SECONDARY_NAME_SLUG).done(function(response) {
        let primaryRaiders = guildMembers.filter(function(member) {
            return member.rank === Roles.Raider;
        });
        
        let secondaryRaiders = response.members.filter(function(member) {
            return member.rank === Roles.SecondaryRaider;
        });
        
        secondaryRaiders.forEach(m => m.faction = response.guild.faction.type);
        
        let raiders = primaryRaiders.concat(secondaryRaiders);
    
        SetRoleDiv(raiders, "raider");
    })
    .fail(function(xhr, textStatus, errorThrown ) {
        if (retries > 0) {
            setTimeout(GetMembersData(guildMembers, retries-1), 1000);
            return;
        }
        return;
    });
...
function GetEncounterData(member, characterProfileSummaryData, retries) {
    ...
    let memberNameElement = document.getElementById("member-name-div-" + member.character.name);

    let itemLevelDiv = document.createElement("h16");
    itemLevelDiv.innerHTML = "&nbsp;&nbsp;&nbsp;(" + characterProfileSummaryData.equipped_item_level + ")";
    itemLevelDiv.classList.add("text");
    memberNameElement.appendChild(itemLevelDiv);
    ...
    GetCharacterEncounterData(member).done(function(characterEncounterData) {
        ...
        SetCharacterProfileCells(characterProfileSummaryData, memberName, member.character.realm.slug, raidProgress, RETRIES);
    })
    .fail(function (xhr) {
        if (retries > 0) {
...
function SetCharacterProfileCells(profileData, memberName, realmSlug, raidProgress, retries) {
    let table = document.getElementById(CHARACTER_TABLE_PREFIX + memberName);
    
    GetCharacterMythicKeystoneSeasonData(memberName, realmSlug)
